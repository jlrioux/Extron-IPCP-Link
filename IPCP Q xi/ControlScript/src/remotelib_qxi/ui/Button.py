...
class ObjectWrapper(ObjectClass):
    ...
    def __init__(self,p,alias,data):
        ...
        """
        event_attrs = ['Pressed','Released','Repeated','Tapped','Held']
    ...
    def receive_message(self,data:'dict'):
        ...
        if data['type'] == 'init':
            ...
            print('enter button query for property {}'.format(data['property']))
            if hasattr(self,data['property']):
                attr = getattr(self,data['property'])
                value = None
                if callable(attr):
                    ...
                    print('property {} is not callable'.format(data['property']))
                    try:
