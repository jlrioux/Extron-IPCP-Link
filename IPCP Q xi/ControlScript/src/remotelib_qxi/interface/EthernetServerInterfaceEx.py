...
from extronlib.interface import EthernetServerInterfaceEx as ObjectClass
...
class ObjectWrapper(ObjectClass):
    ...
    def __init__(self,p,alias,data):
        ...
        self.set_get_attrs = ['Clients','IPPort','Interface','MaxClients','Protocol']
        self.callable_attrs = {'Disconnect':self._Disconnect,
                                 'SSLWrap':None,
                                 'StartListen':None,
                                 'StopListen':None,
                                 'Send':self.Send}
    ...
    def create_event_handler(self,property):
        from extronlib.interface import ClientObject
        def e(client:'ClientObject',*args):
            qualifier = None
            ...
            if property in ['ReceiveData','Connected','Disconnected']:
                client_data={'Hostname':client.Hostname,
    ...
    def Send(self,client,data):
        ...
        for c in self.Clients:
            if client['Hostname'] == c.Hostname and client['IPAddress'] == c.IPAddress and client['ServicePort'] == c.ServicePort:
                c.Send(data)
                return
        msg='{}:Send Failed to client: no such client exists'.format(self.alias,client)
        err_msg = {'property':'Send','value':client,'qualifier':{'code':msg}}
        self.WrapperBasics.send_message(self.alias,json.dumps({'type':'error','message':err_msg}))
        self.WrapperBasics.log_error('remotelib error:{}:{}'.format(self.alias,json.dumps(err_msg)))
