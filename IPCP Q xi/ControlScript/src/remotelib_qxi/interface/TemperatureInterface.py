...
if qxi_flag:
    from extronlib.interface import TemperatureInterface as ObjectClass
...
if qxi_flag:
    class ObjectWrapper(ObjectClass):
        ...
        def __init__(self,p,alias,data):
            ...
            """
            event_attrs = ['Online','Offline','TemperatureChanged','TemperatureStateChanged']
            self.set_get_attrs = ['Host','LimitThreshold','OverThreshold','Port','Temperature','TemperatureState']
    ...
    class ObjectWrapper():
        ...
        def __init__(self,p,alias,data):
            ...
            print('failed to create {} "{}" with args "{}" with exception: Object type not compatible with non-qxi'.format(ObjectWrapper.type,self.alias,self.args))
            msg='failed to create {} "{}" with args "{}"\nwith exception: Object type not compatible with non-qxi'.format(self.type,self.alias,self.args)
            err_msg = {'property':'init','value':self.args,'qualifier':{'code':msg}}
            self.WrapperBasics.send_message(self.alias,json.dumps({'type':'error','message':err_msg}))
            self.WrapperBasics.log_error('remotelib error:{}:{}'.format(self.alias,json.dumps(err_msg)))
            self.initialized = True
        ...
        def receive_message(self,data:'dict'):
            ...
            if data['type'] == 'init':
                ...
                err_msg = {'property':data['property'],'value':None,'qualifier':{'code':'Object type not compatible with non-qxi'}}
            if err_msg:
                if 'query id' in data:self.WrapperBasics.send_message(self.alias,json.dumps({'type':'error','query id':data['query id'],'message':err_msg}))
                else:self.WrapperBasics.send_message(self.alias,json.dumps({'type':'error','message':err_msg}))
                self.WrapperBasics.log_error('remotelib error:{}:{}'.format(self.alias,json.dumps(err_msg)))
